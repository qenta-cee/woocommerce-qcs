...
/**
 * Class WC_Gateway_Wirecard_Checkout_Seamless_Installemnt
 *
 * @since 1.0.0
 */
class WC_Gateway_Wirecard_Checkout_Seamless_Installment {
	...
	/**
	 * WC_Gateway_Wirecard_Checkout_Seamless_Installment constructor.
	 *
	 * @since 1.0.0
	 *
	 * @param $settings
	...
	public function get_label() {
		return __( 'Installment', 'woocommerce-wirecard-checkout-seamless' );
	}
	...
	public function get_payment_fields() {
		...
		$html .= "<select name='wcs_installment_dob_day' class=''>";
		...
		$html .= "<select name='wcs_installment_dob_month' class=''>";
		for ( $month = 12; $month > 0; $month -- ) {
			$html .= "<option value='$month'> $month </option>";
		}
		$html .= "</select>";

		$html .= "<select name='wcs_installment_dob_year' class=''>";
		...
		if ( $this->_settings['woo_wcs_payolutionterms'] && $this->_settings['woo_wcs_installmentprovider'] == 'payolution' ) {
	...
	public function get_payment_type() {
		return WirecardCEE_QMore_PaymentType::INSTALLMENT;
	}
	...
	public function get_risk() {
		if ( $this->_settings['woo_wcs_installmentprovider'] == 'payolution' ) {
			return $this->is_available_payolution();
		} elseif ( $this->_settings['woo_wcs_installmentprovider'] == 'ratepay' ) {
			return $this->is_available_ratepay();
		} elseif ( $this->_settings['woo_wcs_installmentprovider'] == 'wirecard' ) {
	...
	private function is_available_payolution() {
		...
		if (
			! is_array( $this->_settings['woo_wcs_installment_accepted_currencies'] )
			|| ! in_array( get_woocommerce_currency(), $this->_settings['woo_wcs_installment_accepted_currencies'] )
		) {
			return false;
		}

		// if cart total is smaller than set limit
		if ( $cart->total <= floatval( $this->_settings['woo_wcs_installment_min_amount'] ) ) {
			return false;
		}

		// if cart total is greater than set limit
		if (
			floatval( $this->_settings['woo_wcs_installment_max_amount'] ) != 0
			&& $cart->total >= floatval( $this->_settings['woo_wcs_installment_max_amount'] )
		) {
			return false;
		}

		// if cart items count is smaller than set limits
		if ( $cart->get_cart_contents_count() < intval( $this->_settings['woo_wcs_installment_min_basket_size'] ) ) {
			return false;
		}

		// if cart items count is greater than set limit
		if (
			floatval( $this->_settings['woo_wcs_installment_max_basket_size'] ) != 0
			&& $cart->get_cart_contents_count() >= floatval( $this->_settings['woo_wcs_installment_max_basket_size'] )
		) {
			return false;
		}
		...
		foreach ( $cart->cart_contents as $hash => $item ) {
			...
			// if the product is in the "digital goods" category, do not show installment as payment method
		...
		if ( $this->_settings['woo_wcs_installment_billing_shipping_equal'] ) {
		...
		if ( ! in_array( $shipping_address->country,
		                 $this->_settings['woo_wcs_installment_allowed_shipping_countries'] )
		) {
			return false;
		}
		...
		if ( ! in_array( $shipping_address->country,
		                 $this->_settings['woo_wcs_installment_allowed_billing_countries'] )
		) {
			return false;
		}
	...
	public function validate_payment_fields( $data ) {
		...
		$birthdate = new DateTime( $data['wcs_installment_dob_year'] . '-' . $data['wcs_installment_dob_month'] . '-' . $data['wcs_installment_dob_day'] );
