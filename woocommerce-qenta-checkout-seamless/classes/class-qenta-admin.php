...
class WC_Gateway_Qenta_Checkout_Seamless_Admin {
	...
	public function print_admin_form_fields( $gateway ) {
		...
		<nav class="nav-tab-wrapper woo-nav-tab-wrapper wcs-tabs">
			...
			<a href="javascript:void(0);" data-target="#alternativepayments"
			   class="nav-tab "><?= __( 'Alternative payments', 'woocommerce-qenta-checkout-seamless' ) ?></a>
	...
	public function include_backend_header( $gateway ) {
		...
		<script src='<?= plugins_url( 'woocommerce-qenta-checkout-seamless/assets/scripts/admin.js' ) ?>'></script>
		...
		<div class="woo-wcs-settings-header-wrapper">
			<img src="<?= plugins_url( 'woocommerce-qenta-checkout-seamless/assets/images/qenta-logo.png' ) ?>">
			<p><?= __( 'Qenta - Your Full Service Payment Provider - Comprehensive solutions from one single source',
		...
		<div class="woo-wcs-settings-header-wrapper">
			<div class="woo-wcs-backend-links">
				<a class="button-primary" id="wcs-transaction-button"
				   href="?page=qenta_transactions_page&transaction_start=1" <?= ( $_GET['page'] == 'qenta_transactions_page' ) ? 'disabled="disabled"' : '' ?>>
					<?= __( 'Transaction overview', 'woocommerce-qenta-checkout-seamless' ) ?>
				</a>

				<a class="button-primary" id="wcs-support-button"
				   href="?page=qenta_support_request" <?= ( $_GET['page'] == 'qenta_support_request' ) ? 'disabled="disabled"' : '' ?>>
					<?= __( 'Contact support', 'woocommerce-qenta-checkout-seamless' ) ?>
				</a>
				<a class="button-primary" id="wcs-settings-button"
				   href="?page=wc-settings&tab=checkout&section=woocommerce_wcs" <?= ( $_GET['page'] == 'wc-settings' ) ? 'disabled="disabled"' : '' ?>>
					<?= __( 'Qenta settings', 'woocommerce-qenta-checkout-seamless' ) ?>
				</a>
			</div>
		</div>
		<?php
	...
	public function print_transaction_table( $transaction, $page ) {
		...
		$back = __( '< Back', 'woocommerce-qenta-checkout-seamless' );
		$next = __( 'Next >', 'woocommerce-qenta-checkout-seamless' );
		...
		if ( $page > 1 ) {
			$prev_page = $page - 1;
			echo "<a class='button-primary' href='?page=qenta_transactions_page&transaction_start=$prev_page'>$back</a>";
		}
		...
		if ( $pages < 5 ) {
			for ( $i = 0; $i < $pages; $i ++ ) {
				...
				$href   = ( $pagenr == $page ) ? 'javascript:void(0)' : "?page=qenta_transactions_page&transaction_start=$pagenr";
		...
		if ( $page < $pages ) {
			$next_page = $page + 1;
			echo "<a class='button-primary' href='?page=qenta_transactions_page&transaction_start=$next_page'>$next</a>";
		}
		?>
		</div></div></div>
		<?php
	...
	public function print_transaction_details( $data ) {
		...
		if ( $data->order_details ) {
			echo '<div class="postbox ">
				<h2 class="wcs-transaction-h2"><span>' . __( 'Qenta order details',
			                                                 'woocommerce-qenta-checkout-seamless' ) . '</span></h2>
				<div class="inside">
				<table>';
		...
		echo '<div class="postbox ">
				<h2 class="wcs-transaction-h2"><span>' . __( 'Payments', 'woocommerce-qenta-checkout-seamless' ) . '</span></h2>
				<div class="inside">
				<table class="wcs-payments-table">
					<tr>
						<th>' . __( 'Number', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Date', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Gateway reference', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Payment state', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Approved', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Deposited', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Currency', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Operations', 'woocommerce-qenta-checkout-seamless' ) . '</th>
					</tr>';
		...
		if ( count( $data->payments ) == 0 ) {
			echo "<tr class='wcs-no-entries'>
					<td colspan='8'>
						...
						" . __( 'No payments available', 'woocommerce-qenta-checkout-seamless' ) . "
					</td>
				  </tr>";
		}
		...
		// credits
		echo '<div class="postbox ">
				<h2 class="wcs-transaction-h2"><span>' . __( 'Credits', 'woocommerce-qenta-checkout-seamless' ) . '</span></h2>
				<div class="inside">
				<table class="wcs-payments-table">
					<tr>
						<th>' . __( 'Number', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Date', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Gateway reference', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Credit state', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Amount', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Currency', 'woocommerce-qenta-checkout-seamless' ) . '</th>
						<th>' . __( 'Operations', 'woocommerce-qenta-checkout-seamless' ) . '</th>
					</tr>';
		...
		if ( count( $data->credits ) == 0 ) {
			echo "<tr class='wcs-no-entries'>
					<td colspan='8'>
						<span class='dashicons dashicons-warning'></span><br>
						" . __( 'No credits available', 'woocommerce-qenta-checkout-seamless' ) . "
					</td>
				  </tr>";
		}
		...
		<div class="wrap woocommerce">
			<div class="postbox">
				<h2 class="wcs-transaction-h2">
					<span>
						<?= __( 'Support Request', 'woocommerce-qenta-checkout-seamless' ) ?>
					</span>
				</h2>
				<div class="inside">
					<?php
					...
					<form action="?page=qenta_support_request" method="post" name="support-request-form">
							...
							<tr>
								<td class="titledesc support-label">
									<label for="support-mail">
										<?= __( 'To:', 'woocommerce-qenta-checkout-seamless' ) ?>
									</label>
								</td>
								<td class="forminp"><select name="support-mail">
										<option value="support@qenta.com">
											<?= __( 'Support Team Qenta CEE, Austria',
							...
							<tr>
								<td class="titledesc support-label">
									<label for="customer-mail">
										<?= __( 'Your e-mail address:', 'woocommerce-qenta-checkout-seamless' ) ?>
									</label>
								</td>
							...
							<tr>
								<td class="titledesc support-label">
									<label for="support-message">
										<?= __( 'Your message:', 'woocommerce-qenta-checkout-seamless' ) ?>
									</label>
								</td>
						...
						<input type="submit" class="button-primary" name="send-request"
						       value="<?= __( 'Send your request', 'woocommerce-qenta-checkout-seamless' ) ?>"/>
					</form>
				</div>
			</div>
		</div>
